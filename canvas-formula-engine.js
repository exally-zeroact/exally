...
for(var r=s.r;r<=e.r;r++)
for(var c=s.c;c<=e.c;c++)
vals.push(hf.getCellValue({sheet,row:r,col:c}));
return vals;
}
...
if(mMd){var m2=mMd[1].match(/^([A-Z]+\d+):([A-Z]+\d+)$/i);if(m2){var s=_toRC(m2[1]),e=_toRC(m2[2]),n=e.r-s.r+1,mat=[];for(var r=0;r<n;r++){mat.push([]);for(var c=0;c<n;c++)mat[r].push(hf.getCellValue({sheet,row:s.r+r,col:s.c+c})||0);}return String(Math.round(_jsMdeterm(mat)*10000)/10000);}}
