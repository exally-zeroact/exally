...
for (let si = 0; si < wb.SheetNames.length; si++) {
  ...
  /* ★★もう1回 打ち直します★★（2026-09-15）
     ★打つ 順が 頼りの 順と 違うと、先に 打った 式が ★古い 材料★で 答えを 出します★。
     ★台は 頼りを 追って 直す 作り★ですが ★それが 本当に 効いて いるかは 別★＝
     ⇒★2回目を 打って 数が 変わるか 見ます★
       ・★変われば 私の 押し方の 穴★（順番）
       ・★変わらなければ 台の 中身の 違い★
     ★これを しないと「台が 悪い」と 言えません★ */
  if (!process.env.IKKAI === '1') {
    for (const a of Object.keys(式たち)) {
      if (板か(式たち[a]) || 染[a]) continue;
      try { 表.打つ(a, 式たち[a]); } catch (e) { /* 何も しない */ }
    }
  }
