...
/* ★★本を 丸ごと 1つの 表に 建てる★★（2026-09-15・㍸の【１段目】）
     ★前は 板ごとに 別の 表を 建てて いました★＝★板を またぐ 式は 押せない★
     ★【１段目】では 押す 選び方を 変えません★
       ＝★ここで 数が 変わったら「建て方を 変えた せい」★（★原因を 1つに★）
     ★--matagu を 付けると【２段目】★＝板またぎ・染まりも 押します */
const またぐも = process.argv.includes('--matagu');
const { 板ごと } = 土台.本を押す(wb, 直し, 逆, またぐも);

for (const 板 of 板ごと) {
  const { 式たち, 値たち, 染, 表 } = 板;
  const si = 板.si;
  /* ★★番地に 板を 付けて 引く★★＝★裸の `A1` だと ★最後に 押した 板★ に なる★ */
  const 引く = (a) => 板.名 + '!' + a;
  ...
  for (const a of Object.keys(式たち)) {
    ...
    if (!またぐも && (ま || 染[a])) continue;   /* ★【１段目】は 押さない★ */
    押した++;

    let 出;
    try { 出 = String(表.字(引く(a))); } catch (e) { 出 = '★落ちた★'; }
    if (出 === '★落ちた★') continue;
