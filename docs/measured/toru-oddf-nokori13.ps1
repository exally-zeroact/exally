# toru-oddf-nokori13.ps1 -- ★ODDF の 別の 2組 と 部品 4本★（㈒）（2026-09-19）
#
#  ★出どころ★
#    組B ... ★紙から 機械で 引きました★ ... golden-kansuu-7kaime-2026-09-18.tsv
#           ★実Excel の 答え（★対照★）... 103.37232293583249★
#           ⇒★この 1本が 同じ 数に ならなければ この 回は 信じられません★
#    ★組A ... ★どの 紙にも 在りません★★
#           ＝★便り（Exally1）だけが 出どころ★
#           ⇒★私が 1回 写して います＝★写し違いの 恐れが 在ります★
#           ⇒★★Exally1 は 出た 数を 見る 前に 式を 見て ください★★
#    ★部品 4本 ... 組A の 決済/満期/頻度/basis から 機械で 作りました★
#
...
$出 = Join-Path $ここ 'golden-oddf-nokori13-2026-09-19.tsv'
...
# ══ ★紙から 拾った 問い★ ══
$式たち = @(
  @{ 番 = '1'; 式 = '=ODDFPRICE(DATE(2008,1,15), DATE(2012,6,30), DATE(2007,11,20), DATE(2008,6,30), 0.045, 0, 100, 2, 1)' },
  @{ 番 = '2'; 式 = '=ODDFPRICE(DATE(2008,1,15), DATE(2012,6,30), DATE(2007,11,20), DATE(2008,6,30), 0, 0.05, 100, 2, 1)' },
  @{ 番 = '3'; 式 = '=ODDFPRICE(DATE(2008,1,15), DATE(2012,6,30), DATE(2007,11,20), DATE(2008,6,30), 0.045, 0.05, 100, 2, 1)' },
  @{ 番 = '4'; 式 = '=ODDFPRICE(DATE(2009,3,1), DATE(2013,1,1), DATE(2009,1,1), DATE(2010,1,1), 0.06, 0, 100, 2, 1)' },
  @{ 番 = '5'; 式 = '=ODDFPRICE(DATE(2009,3,1), DATE(2013,1,1), DATE(2009,1,1), DATE(2010,1,1), 0, 0.05, 100, 2, 1)' },
  @{ 番 = '6'; 式 = '=ODDFPRICE(DATE(2009,3,1), DATE(2013,1,1), DATE(2009,1,1), DATE(2010,1,1), 0.06, 0.05, 100, 2, 1)' },
  @{ 番 = '7'; 式 = '=COUPDAYS(DATE(2008,1,15), DATE(2012,6,30), 2, 1)' },
  @{ 番 = '8'; 式 = '=COUPPCD(DATE(2008,1,15), DATE(2012,6,30), 2)' },
  @{ 番 = '9'; 式 = '=COUPNCD(DATE(2008,1,15), DATE(2012,6,30), 2)' },
  @{ 番 = '10'; 式 = '=COUPDAYBS(DATE(2008,1,15), DATE(2012,6,30), 2, 1)' }
)
