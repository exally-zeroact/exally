# toru-formulatext-wo-excel-ni-hirakaseru.ps1
#   -- ★FORMULATEXT の 前（裸）と 後（_xlfn.）を 実Excel に 開かせる★（60）（2026-09-20）
#
#  ★★なぜ★★
#    59 で 実Excel 自身に 聞いた 所 `_xlfn.FORMULATEXT(B1)`。
#    うちの 名簿には ★無かった★＝★裸で 書いて いた★＝★4個目の 穴★。
#    ⇒★但し 「#NAME? に なる」は ★見立て★でした★
#    ⇒★ここで 割ります★
#
#  ★★前の 3個との 違い（★重さ★）★★
#    `WRAPROWS` `WRAPCOLS` `MODE.MULT` ... ★溢れる★⇒★表が 丸ごと 消えた★
#    `FORMULATEXT` .................... ★溢れない（cm 無し）★
#    ⇒★1マスだけ 割れる はず★＝★ここも 確かめます（C2 が 空か）★
#
#  ★★開く 物（★字で 書いた 2本だけ★）★★
#    前 `%TEMP%\exally-kakidashi-formulatext0.xlsx` 16,236B（`FORMULATEXT(B1)`＝裸）
#    後 `%TEMP%\exally-kakidashi-formulatext.xlsx`  16,242B（`_xlfn.FORMULATEXT(B1)`）
#    ★対照（前後とも 同じ）★
#      B1  `SUM(A1:A3)` ⇒ 6（★FORMULATEXT が 読む 相手★）
#      E1  `SUMPRODUCT(A1:A3,A1:A3)` ⇒ 14（★裸で 正しい★）
#      G1  `_xlfn.SEQUENCE(3)` ⇒ G1:G3（★付く★）
#      A10 `SUM(A1:A3)` ⇒ 6（★溢れない★）
#    ・★司さんの 実物の 名は 1文字も 在りません★／★読むだけ★
#
#  ★★4問★★
#    ⑴前の C1 が ★#NAME?★ か
#    ⑵後の C1 が ★`=SUM(A1:A3)` の 字★ に なるか
#    ⑶★C2 が 空か★＝★溢れない のを 確かめる★（重さの 裏取り）
#    ⑷対照 4つが 前後とも 同じか
#
#  ★門★
...
$出 = Join-Path $ここ 'golden-formulatext-excel-2026-09-20.tsv'

# ★★2026-09-20 足し ── ★前（裸）も 置かれた ので 2本立てに します★★
#   ＝★同じ 道具・同じ 1回で 比べる★（別々に 測ると 「道具が 変わった」が 混ざる）
$二本 = @(
  @{ 札 = 'mae(hadaka)';            名 = 'exally-kakidashi-formulatext0.xlsx' },
  @{ 札 = 'ato(_xlfn. wo tashita)'; 名 = 'exally-kakidashi-formulatext.xlsx' }
)
...
try {
  ...
  $行.Add('# ★FORMULATEXT の 前（裸）と 後（_xlfn.）を 実Excel に 開かせた★（60）（2026-09-20）')
  $行.Add('# ★読むだけ★（保存して いません）／★開けるのは 字で 書いた 2本だけ★')
  $行.Add('# ★どの Excel か★ ... 版 ' + $xl.Version + ' ／ build ' + $xl.Build)
  $行.Add('# ★どの 貝殻か★ ... PowerShell ' + $版.ToString())
  $行.Add('# ★生の 字★ C1 `FORMULATEXT(B1)`（裸）⇒ `_xlfn.FORMULATEXT(B1)`')
  $行.Add('# ★対照★   B1 `SUM(A1:A3)`＝6／E1 `SUMPRODUCT`＝14（裸で 正しい）／G1 `_xlfn.SEQUENCE(3)`／A10 `SUM`＝6')
  $行.Add('# ★実Excel 自身は★ `_xlfn.FORMULATEXT` と 書きます（59 で 実測・★cm 無し＝溢れない★）')
