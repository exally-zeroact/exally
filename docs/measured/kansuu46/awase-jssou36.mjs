...
function 土台() {
  /* ★実Excel に 聞いた 時と ★同じ 土台★（toru-jssou36.ps1 の 頭に 書いた）★
     A1:A5=1..5 / B1:B5=2,4,6,8,10 / C1=0 C2=3 C3=0.1 C4=0.2 C5=1
     F1:I3=台 / K1:K2=条件 */
  const 表 = [];
  for (let r = 0; r < 6; r++) 表.push(new Array(16).fill(null));
...
function 押す(式) {
  ...
  try {
    ...
    if (typeof EF._hfGetDisplay === 'function') return EF._hfGetDisplay(0, 0, 13, true);
    const v = hf.getCellValue({ sheet: SID, row: 0, col: 13 });
