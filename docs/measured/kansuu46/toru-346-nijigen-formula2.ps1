...
try {
  ...
  $行.Add('# ★★元の 紙は `.Formula`（昔の 道＝溢れない）で 取られて います★★（kansuu46 の 道具 21本とも）')
  ...
  $行.Add('# ★取り直した 訳★ ... 古い 紙は `.Formula`（昔の 道＝溢れない）で 取って いる')
  $行.Add('# ★古い 紙は 1文字も 触って いません★（昔の 道の 答えも 残す）')
  $行.Add('# ★どの Excel か★ ... 版 ' + $xl.Version + ' ／ build ' + $xl.Build)
  $行.Add('# ★どの 貝殻か★ ... PowerShell ' + $版.ToString())
  $行.Add('# 名前' + "`t" + '式' + "`t" + '昔の 答え' + "`t" + '今の 答え' + "`t" + '型' + "`t" + '=(マス)=0' + "`t" + '埋まった数' + "`t" + '判じ' + "`t" + '中身（|で マス／ / で 行）')
