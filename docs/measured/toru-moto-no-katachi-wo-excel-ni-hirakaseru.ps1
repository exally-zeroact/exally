...
#   -- ★元の ファイルの 図形（判子）の 位置を 実Excel に 聞く★（77）（2026-09-21）
#
#  ★★なぜ★★
...
#
#  ★★開く 物★★ `%TEMP%\exally-tameshi-hiraku3.xlsx`（★1本の 名★）
...
$出 = if ($わざと) { Join-Path $ここ 'golden-moto-no-katachi-excel-wazato-2026-09-21.tsv' }
      else { Join-Path $ここ 'golden-moto-no-katachi-excel-2026-09-21.tsv' }
...
try {
  ...
  $行.Add('# ★元の ファイルの 図形（判子）の 位置を 実Excel に 聞いた★（77）（2026-09-21）')
