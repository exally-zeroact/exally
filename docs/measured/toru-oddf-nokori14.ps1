# toru-oddf-nokori14.ps1 -- ★組3 の 近所 3組 と 部品 2本★（㈓）（2026-09-19）
#
#  ★出どころ★ ... Exally1 の 便り
#    ★紙には まだ 在りません★ = ★私が 1回 写して います★
#    ⇒★★数を 見る 前に 式を 見て ください★★
#
#  ★対照★ ... golden-oddf-nokori-2026-09-18.tsv
#    実Excel の 答え = ★96.1767213744142★
#    ⇒★この 1本が 同じ 数に ならなければ この 回は 捨てます★
#
#   1  3a 利率0
#   2  3a 利回り0
#   3  3a 両方
...
#  10  部品 3b の COUPDAYS
#  11  部品 3c の COUPDAYS
#  12  対照 (紙 golden-oddf-nokori-2026-09-18.tsv = 96.1767213744142)
#
#
...
$出 = Join-Path $ここ 'golden-oddf-nokori14-2026-09-19.tsv'
...
# ══ ★紙から 拾った 問い★ ══
$式たち = @(
  @{ 番 = '1'; 式 = '=ODDFPRICE(DATE(2009,3,10), DATE(2015,2,28), DATE(2008,12,5), DATE(2009,8,30), 0, 0.05, 100, 4, 1)' },
  @{ 番 = '2'; 式 = '=ODDFPRICE(DATE(2009,3,10), DATE(2015,2,28), DATE(2008,12,5), DATE(2009,8,30), 0.045, 0, 100, 4, 1)' },
  @{ 番 = '3'; 式 = '=ODDFPRICE(DATE(2009,3,10), DATE(2015,2,28), DATE(2008,12,5), DATE(2009,8,30), 0.045, 0.05, 100, 4, 1)' },
  @{ 番 = '4'; 式 = '=ODDFPRICE(DATE(2009,3,10), DATE(2015,5,31), DATE(2008,12,5), DATE(2009,8,31), 0, 0.05, 100, 4, 1)' },
  @{ 番 = '5'; 式 = '=ODDFPRICE(DATE(2009,3,10), DATE(2015,5,31), DATE(2008,12,5), DATE(2009,8,31), 0.045, 0, 100, 4, 1)' },
  @{ 番 = '6'; 式 = '=ODDFPRICE(DATE(2009,3,10), DATE(2015,5,31), DATE(2008,12,5), DATE(2009,8,31), 0.045, 0.05, 100, 4, 1)' },
  @{ 番 = '7'; 式 = '=ODDFPRICE(DATE(2009,6,10), DATE(2015,2,28), DATE(2008,12,5), DATE(2009,8,31), 0, 0.05, 100, 4, 1)' },
  @{ 番 = '8'; 式 = '=ODDFPRICE(DATE(2009,6,10), DATE(2015,2,28), DATE(2008,12,5), DATE(2009,8,31), 0.045, 0, 100, 4, 1)' },
  @{ 番 = '9'; 式 = '=ODDFPRICE(DATE(2009,6,10), DATE(2015,2,28), DATE(2008,12,5), DATE(2009,8,31), 0.045, 0.05, 100, 4, 1)' },
  @{ 番 = '10'; 式 = '=COUPDAYS(DATE(2009,3,10), DATE(2015,5,31), 4, 1)' },
  @{ 番 = '11'; 式 = '=COUPDAYS(DATE(2009,6,10), DATE(2015,2,28), 4, 1)' },
  @{ 番 = '12'; 式 = '=ODDFPRICE(DATE(2009,3,10), DATE(2015,2,28), DATE(2008,12,5), DATE(2009,8,31), 0.045, 0.05, 100, 4, 1)' }
)
