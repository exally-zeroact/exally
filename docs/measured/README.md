...
## ★★実Excel に 聞く 前の 決まり★★（2026-09-16 に 足しました）
...
```
  ★答えを 見て から 形を 寄せない★
  ★なぜ★
    2026-09-16 に ODDFPRICE の 式を ★当てて 6通り 試し★、
    ★全部 元より 悪く★ なりました（棚63）。
    ★当てる★ とは ★答えを 見てから 形を 寄せる★ 事です。
  ★手順★
    ①★聞く 組と 見込みを 紙に 書く★（★先に commit する★）
      … 例 `docs/measured/kansuu46/oddf-kiku-koto.md`
    ②実Excel に 聞く
    ③★★「当たった／外れた」を 先に 書く★★
      … 例 `docs/measured/kansuu46/oddf-kiita-kekka.md`
    ④★その 後で 直しを 考える★
  ⇒★★順を 逆に すると 辯褜合わせに なります★★
```

### ㆂ★必ず ★対照★を 1本 入れる★

```
  ★紙に 既に 在って ○ の 物を もう 一度 打つ★
  ⇒★合わなければ ★測り台そのものが 変わって います★★
    ＝★他の 答えも 信じられません★⇒★そこで 止める★
  ★対照を 作る 時も ★引数の 順番を 確かめる★
    （2026-09-16 に 私は ODDLPRICE の 対照を
     ★最終利払日を 決済日より 後★ に して #NUM! に しました）
```

### ㆃ★★1本 打てなくても 全部 止めない★★

```
  ★2026-09-16 に 実際に 起きました★
    `=CALL("x")` で `HRESULT 0x800A03EC`（★Excel が 式を 受け付けない★）
    ⇒★その 1本で ★307秒 掛けた 枛が 丸ごと 無駄★に なりました★
  ⇒★★1本ずつ 受け止めて「打てません」と 控える★★
  ★「打てない」も 測った 結果です★（★捨てて は いけません★）
```

### ㆄ★「空の 字」と「測れて いない」は 別物★

```
  2026-09-16 … PHONETIC と PIVOTBY は ★空の 字を 返しました★（型は String）
  ⇒★これも 測った 結果★です。★空だから と 「測れて いない」と 数えない★
  ⇒★見分けるのは ★型の 欄★★（型が 空なら 測れて いません）
```
...
```
  ・走らせる ★その時に★ Excel を ┅2つの 道具で 数える★（1個でも 居たら 走らせない）
    ＝★人の 0個を 使い回さない★
  ・★新しい 空の ブックだけ★（★司さんの 実物は 開かない★）
  ・Visible=$false ／ DisplayAlerts=$false ／ finally で 必ず Quit
  ・★消えるまで 待って 秒数を 出す★（実測 59秒の 例が 在る＝10秒の 見張りは 嘘の 赤）
  ・★`.ps1` は BOM 必須★
  ・★2つ目の 窓（=(式)=0）と 型★も 取る
    ＝`.Value2` は ★0 で ない 値に 0 を 返す★ので 1つの 窓では 見分けられません
```
...
## `exally-missing-2026-09-07.txt` ... ★Exally で 動かない ★19個★★（09-06 の 65個から 46個 減った／★2026-09-19 に ODDL 2個・ODDF 2個を 降ろした＝保留 0個★／★同じ日に BYROW・BYCOL・LAMBDA・PHONETIC の 4個も 降ろした＝見張りが ★2段しか 押して いなかった★ので「動かない」と 書いて いた★）
