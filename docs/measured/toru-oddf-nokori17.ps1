# toru-oddf-nokori17.ps1 -- ★測り直し ⬚ ㈖★（2026-09-19）
#
#  ★わざと 前回と 順を 変えて います★ = ★行が ずれて いない 事の 証し★
#  ★対照★ ... golden-oddf-nokori-2026-09-18.tsv = ★96.1767213744142★
#
#  1  A ket 06-01 riritsu0 (mae 4banme)
#  2  B hak 01-05 mawari0
#  3  A ket 05-30 riritsu0 (mae 2banme)
#  4  B hak 01-05 ryouhou
#  5  A ket 03-10 riritsu0 (mae 1banme)
#  6  B hak 01-05 riritsu0
#  7  A ket 06-10 riritsu0 (mae 5banme)
#  8  taisho (golden-oddf-nokori-2026-09-18.tsv = 96.1767213744142)
#  9  A ket 05-31 riritsu0 (mae 3banme)
#
#
...
$出 = Join-Path $ここ 'golden-oddf-nokori17-2026-09-19.tsv'
...
# ══ ★紙から 拾った 問い★ ══
$式たち = @(
  @{ 番 = '1'; 式 = '=ODDFPRICE(DATE(2009,6,1), DATE(2015,2,28), DATE(2008,12,5), DATE(2009,8,31), 0, 0.05, 100, 4, 1)' },
  @{ 番 = '2'; 式 = '=ODDFPRICE(DATE(2009,3,1), DATE(2013,1,1), DATE(2009,1,5), DATE(2010,1,1), 0.06, 0, 100, 2, 1)' },
  @{ 番 = '3'; 式 = '=ODDFPRICE(DATE(2009,5,30), DATE(2015,2,28), DATE(2008,12,5), DATE(2009,8,31), 0, 0.05, 100, 4, 1)' },
  @{ 番 = '4'; 式 = '=ODDFPRICE(DATE(2009,3,1), DATE(2013,1,1), DATE(2009,1,5), DATE(2010,1,1), 0.06, 0.05, 100, 2, 1)' },
  @{ 番 = '5'; 式 = '=ODDFPRICE(DATE(2009,3,10), DATE(2015,2,28), DATE(2008,12,5), DATE(2009,8,31), 0, 0.05, 100, 4, 1)' },
  @{ 番 = '6'; 式 = '=ODDFPRICE(DATE(2009,3,1), DATE(2013,1,1), DATE(2009,1,5), DATE(2010,1,1), 0, 0.05, 100, 2, 1)' },
  @{ 番 = '7'; 式 = '=ODDFPRICE(DATE(2009,6,10), DATE(2015,2,28), DATE(2008,12,5), DATE(2009,8,31), 0, 0.05, 100, 4, 1)' },
  @{ 番 = '8'; 式 = '=ODDFPRICE(DATE(2009,3,10), DATE(2015,2,28), DATE(2008,12,5), DATE(2009,8,31), 0.045, 0.05, 100, 4, 1)' },
  @{ 番 = '9'; 式 = '=ODDFPRICE(DATE(2009,5,31), DATE(2015,2,28), DATE(2008,12,5), DATE(2009,8,31), 0, 0.05, 100, 4, 1)' }
)
