...
| ② | **.xlsb を読める** | Claude.ai の取り込みは ★.xlsx 30MBまで・.xlsb は 一覧に無い★ | ★.xlsb を そのまま読む★ | ★司さんの実物 .xlsb を そのまま読む★（2026-08-29 実測＝★式 19,323本／表の参照を 直した所 15,734本★）／★計算の道具を 2.6.1→3.4.0 に 入れ替えても 答えが 変わった式は 0本★<br>★Excelが ファイルに 残した答えと ★19,323/19,323（100%）一致★（★本番の道★＝lib/table-refs.js → convertFormula → シート名を囲む → ★Exally関数プラグイン★ → HyperFormula 3.4.0。2026-08-29）<br>★100% と 言えるまでに 直した 4つ（全部 実Excel 16.0 で 真値を 測ってから 直した）★＝①空のセルを指した式は 0（2,918本）／②.xlsb の 配列の式（記録426）を 拾う（36本）／③`_xlws.` など Excel内部の印を 外す／④★FILTER が 形を保つ★＝縦の条件は 行を選ぶ・横の条件は 列を選ぶ（2列以上で 中身も形も 壊れていた）<br>★この数は 1つのブックの数字★＝他のブックで 100% とは 言わない |
