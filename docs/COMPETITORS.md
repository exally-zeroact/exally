...
| ② | **.xlsb を読める** | Claude.ai の取り込みは ★.xlsx 30MBまで・.xlsb は 一覧に無い★ | ★.xlsb を そのまま読む★ | ★司さんの実物 .xlsb を そのまま読む★（2026-08-29 実測＝★式 19,323本／表の参照を 直した所 15,734本★）／★計算の道具を 2.6.1→3.4.0 に 入れ替えても 答えが 変わった式は 0本★<br>★「一致率100%」とは 言わない★＝Excelが ファイルに 残した答えと 合ったのは ★16,353/19,323（84.6%）★（★本番の道★＝convertFormula＋★Exally関数プラグイン★を 通して 測った値。2026-08-29）<br>★合わなかった 2,970本の 中身★＝①★2,918本＝空のセルを 指した時★（Excelは 0・うちは 空。流儀の違い）／②★52本＝.xlsb の 表の参照★（★読み取りライブラリが 列名を 落としている＝うちの計算では ない★）／③★数どうしで 違う＝0本★／④★字で 違う＝0本★<br>★お金の数字が 違った式は 1本も 無い★ |
