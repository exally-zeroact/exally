...
function _jsDbFunc(func,sheet,dbRange,field,criteriaRange){if(!_hf)return'#VALUE!';var m=dbRange.match(/^([A-Z]+\d+):([A-Z]+\d+)$/i);if(!m)return'#VALUE!';var s=_toRC(m[1]),e=_toRC(m[2]);var headers=[];for(var c=s.c;c<=e.c;c++)headers.push(_hf.getCellValue({sheet:_hfSid(sheet),row:s.r,col:c}));var fieldIdx=typeof field==='number'?field-1:headers.indexOf(field);/* ★field が 台帳の 列数より 大きい／0／負★ … 実 Excel は ★#VALUE!★ ★前は 門が `<0` だけで、5 を 渡すと ★台帳の 外の 列★を 読んで ★0★ を 返して いた ★実測★ `=DSUM(A1:B5,5,F1:F2)` … 前 ★0★ ／実 Excel ★#VALUE!★（tests/dkei6-honban.test.mjs の 26行） */if(fieldIdx<0||fieldIdx>=headers.length)return'#VALUE!';var cm=criteriaRange.match(/^([A-Z]+\d+):([A-Z]+\d+)$/i);if(!cm)return'#VALUE!';var cs=_toRC(cm[1]);var critField=_hf.getCellValue({sheet:_hfSid(sheet),row:cs.r,col:cs.c});var critVal=_hf.getCellValue({sheet:_hfSid(sheet),row:cs.r+1,col:cs.c});var critFieldIdx=headers.indexOf(critField);/* ★条件の 見出しが 台帳の 見出しと 紐付かない 時★ … 実 Excel は ★その 条件を 効かせない★（＝全部 数える） ★前は `indexOf` の ★-1★ を そのまま 足して ★範囲の 外（列 -1）を 読んで★ 1件も 当たらなかった ★実測★ `=DSUM(A1:B5,1,D1:D2)` … 前 ★0★ ／ 実 Excel ★14★（tests/dkei-honban.test.mjs の 21行） ★直したのは ここだけ★＝★製品と 台は まだ 6か所 違います（棚 ㉝・実測して いません）★ */var results=[];for(var r=s.r+1;r<=e.r;r++){if(critFieldIdx<0||_jsDbAtehamaru(_hf.getCellValue({sheet:_hfSid(sheet),row:r,col:s.c+critFieldIdx}),critVal)){results.push(_hf.getCellValue({sheet:_hfSid(sheet),row:r,col:s.c+fieldIdx}));}}var nums=results.filter(function(v){return typeof v==='number';});if(func==='SUM')return nums.reduce(function(a,b){return a+b;},0);if(func==='AVG')return nums.length?nums.reduce(function(a,b){return a+b;})/nums.length:'#DIV/0!';if(func==='CNT')return nums.length;if(func==='CNTA')return results.filter(function(v){return v!==null&&v!==undefined;}).length;if(func==='MAX')return nums.length?Math.max.apply(null,nums):'#NUM!';if(func==='MIN')return nums.length?Math.min.apply(null,nums):'#NUM!';if(func==='PROD')return nums.reduce(function(a,b){return a*b;},1);/* ★DGET は 0件と 2件以上で ★誤りが 違います★（実 Excel・実測） 0件 … ★#VALUE!★ ／ 2件以上 … ★#NUM!★ ★前は どちらも #NUM!★（`=DGET(A1:B5,1,F1:F2)` 0件 … 前 #NUM! ／実 Excel #VALUE!） */if(func==='GET')return results.length===1?results[0]:(results.length===0?'#VALUE!':'#NUM!');if(func==='STD'){var n=nums.length;if(n<2)return'#DIV/0!';var mean=nums.reduce(function(a,b){return a+b;})/n;return Math.sqrt(nums.reduce(function(a,v){return a+Math.pow(v-mean,2);},0)/(n-1));}if(func==='STDP'){var n=nums.length;if(!n)return'#DIV/0!';var mean=nums.reduce(function(a,b){return a+b;})/n;return Math.sqrt(nums.reduce(function(a,v){return a+Math.pow(v-mean,2);},0)/n);}if(func==='VAR'){var n=nums.length;if(n<2)return'#DIV/0!';var mean=nums.reduce(function(a,b){return a+b;})/n;return nums.reduce(function(a,v){return a+Math.pow(v-mean,2);},0)/(n-1);}if(func==='VARP'){var n=nums.length;if(!n)return'#DIV/0!';var mean=nums.reduce(function(a,b){return a+b;})/n;return nums.reduce(function(a,v){return a+Math.pow(v-mean,2);},0)/n;}return'#VALUE!';}
...
/* ★★指数の 大文字 E を 小文字 e に する（2026-09-10）★★
 *
 *  ★何が 起きて いたか★
 ...
 *    ⇒★★プラグインでは 直りません（式を 読む 所＝借り物の 中）★★
 *      ⇒★借り物の 中は 読まず、★渡す 前に 書き換える★★
 *
 *  ★「E14 という セル参照と ぶつかって いる」は ★外れ★でした★
 *    押して 確かめました … `=E14` → 999 ／ `=SUM(E1:E14)` → 1002 ★どちらも 正しく 動く★
 *    ⇒ ぶつかって いません。★大文字 E を 数として 読まないだけ★です
 *
 *  ★★一番 危ないのは ★セル参照を 壊す 事★★★
 *    ⇒★壊れる 側から 試験を 書きました★（下の 見張り）
 *    ⇒ 触らない 物 … `=E14` `=SUM(E1:E14)` `=$E$14` `=Sheet1!E14`
 *                    `="1.64E-14"`（★字の 中★） `=TEXT(A1,"0.00E+00")`（★書式の 中★）
 *                    `=A1E5`（★前に 英字＝セルの 名前かも★） `=EXP(2)`（関数名）
 *
 *  ★見て いない 範囲★
 *    ・★出る 字は 直して いません★（うち `1.64e-14` ／ 実Excel `1.64E-14`）＝★別の 直し★
 *    ・★一重引用符（'）の 中は 見て いません★（シート名の 囲み）
 */
function _指数のEを小文字に(f){
  if(!f||f.indexOf('E')<0) return f;
  var 配列=f.split(''), 字の中=false;
  for(var i=0;i<配列.length;i++){
    if(配列[i]==='"'){ 字の中=!字の中; continue; }
    if(字の中) continue;                              /* ★字の 中は 触らない★ */
    if(配列[i]!=='E') continue;
    /* ★E の 前が 数字★（1.64E / 1E / 2E …） */
    var 前=配列[i-1];
    if(!(前>='0'&&前<='9')) continue;
    /* ★その 数字の 頭まで 戻り、その 前に 英字が 在れば セルの 名前★（A1E5 など） */
    var j=i-1;
    while(j>=0&&配列[j]>='0'&&配列[j]<='9') j--;
    if(j>=0&&配列[j]==='.'){ j--; while(j>=0&&配列[j]>='0'&&配列[j]<='9') j--; }
    if(j>=0&&/[A-Za-z_$]/.test(配列[j])) continue;
    /* ★E の 後ろ＝符号? ＋ 数字★（後ろに 英字が 続けば セルの 名前） */
    var k=i+1;
    if(配列[k]==='+'||配列[k]==='-') k++;
    var 数=0;
    while(k<配列.length&&配列[k]>='0'&&配列[k]<='9'){ k++; 数++; }
    if(!数) continue;
    if(k<配列.length&&/[A-Za-z_]/.test(配列[k])) continue;
    配列[i]='e';
  }
  return 配列.join('');
}
