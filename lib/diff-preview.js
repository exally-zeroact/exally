...
(function (root, factory) {
  ...
  function build(o) {
    ...
    for (var i = 0; i < sheets.length; i++) {
      ...
      out.sheets.push({ name: sh.name, count: keys.length, rows: rows, more: Math.max(0, keys.length - rows.length) });
    }
    return out;
  }
  ...
  function headline(plan, fileName) {
    ...
    return (fileName ? fileName + ' に ' : '') + plan.total + 'か所 書き込みます'
      + (n > 1 ? '（' + n + 'つのシートに広がっています）' : '');
  }

  return { build: build, headline: headline, colName: colName, UNKNOWN: UNKNOWN };
}));
