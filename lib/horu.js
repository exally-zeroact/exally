...
(function (root) {
  ...
  function 掘れる回数を決める(sheets, opt) {
    ...
    var 欲しい = Math.max(下限, Math.ceil(全部 / Math.max(1, 一度のセル)) + 2);
    /* ★叩ける 回数を 超えない★（2026-09-05）
       前は ここが ★13★を 返し、book.html が +2 して ★15回★ 叩いていた。
       ところが サーバは ★1分に 10回で 止める★（api/claude.js 事故止め）。
       ⇒★11回目で 止まり、それまでの 掘りを 全部 捨てて「使いすぎ」だけ 出していた★
       ⇒★数は lib/ai-genkai.js の 1か所に 集めた★。ここは そこに 従う。 */
    var G = (typeof module !== 'undefined' && module.exports)
      ? require('./ai-genkai.js')
      : (typeof window !== 'undefined' ? window.AiGenkai : null);
    return G ? G.掘ってよい回数(欲しい) : 欲しい;
  }
  ...
  /** ★客に「全部は 見られていない」と 言う★（2026-09-05）
   *  ★なぜ★ … サーバが 1分に 叩ける 回数（lib/ai-genkai.js）に 頭打ちされ、
   *    大きいブックでは ★全部は 掘りきれない★。
   *    ★AIには「これ以上 掘れない」と 言っていたが、客には 何も 言っていなかった★
   *    ＝★客は「全部 見て 答えた」と 思う★。★これが 一番 危ない★。
   *  ★司さん 2026-08-22 の 決め「何度でも掘れる＝実質 全部見られる」からは 後退★。
   *    ⇒★戻す 手（材料を 足し続けない／掘った分を 使い回す）は 別に 立てる★。
   *    ⇒★それまでは 黙らない★。 */
  function 掘りきれない時の客への言葉(掘った回数, 全セル数) {
    return 'このブックは 大きいので、今回は ' + 掘った回数 + '回ぶん'
      + '（約 ' + (掘った回数 * 決まり.セル数).toLocaleString() + ' セル）まで 見て 答えました。'
      + '全部で ' + Number(全セル数 || 0).toLocaleString() + ' セル 在ります。'
      + '見てほしい シートや 範囲を 言うてもらえたら、そこを 見に行きます。';
  }
