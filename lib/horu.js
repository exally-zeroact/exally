...
(function (root) {
  ...
  var 決まり = {
    /* ★★2026-09-04 に 直した（司さん「なんで できてないんど／何のために 細胞レベルに 分解した」）★★
       ★前★ … ★5回まで★ と 決め打ちだった
              ＝実物 21,204セル ÷ 2,000 ＝ ★半分も 掘れずに 止まっていた★
              ＝正本（2026-08-22）の ★「何度でも掘れる＝実質 全部見られる」★に 反していた
       ★今★ … ★そのブックを 掘りきれる 回数★を その場で 出す（下の 掘れる回数を決める）
              ＝★大きいブックほど 多く 掘れる★／★小さいブックで 無駄に 回さない★
       ★暴走はさせない★＝掘りきったら それ以上は 増えない（無限には ならない）。 */
    掘れる回数: 5,   /* ★下限★（小さいブックでも これだけは 掘れる） */
    セル数: 2000,    /* 1回に返すセルの数 */
    字数: 12000,     /* 1回に返す字の数（2万トークンの内側に収める） */
  };

  /** ★このブックを 掘りきるのに 何回 要るか★（＝掘れる回数の 上限を その場で 出す）
   *  ★決め打ちの 5 では 半分しか 見られない★＝ブックの 大きさから 出す。
   *  ＋2 は ★同じ所を 二度 頼まれた分の 余裕★（掘り直しで 止まらない為）。 */
  function 掘れる回数を決める(sheets, opt) {
    var 一度のセル = (opt && opt.セル数) || 決まり.セル数;
    var 下限 = (opt && opt.掘れる回数) || 決まり.掘れる回数;
    var 全部 = 0;
    for (var i = 0; i < (sheets || []).length; i++) {
      var d = sheets[i] && sheets[i].data;
      if (d) 全部 += Object.keys(d).length;
    }
    return Math.max(下限, Math.ceil(全部 / Math.max(1, 一度のセル)) + 2);
  }
