...
(function (root) {
  ...
  function AIへ渡す物(要約, 頼み) {
    ...
    行.push('          {"種類":"写す","元":"A1:D9","先":"F1","値だけ":true},');
    行.push('          {"種類":"切り出す","人":"（見出しの名前）","月":"2026-01","始まりの日":1} ]}');
    行.push('写す … 同じシートの中だけ。値だけ が true なら 式は 値にして 写します。');
  ...
  var 使える種類 = ['式の列を足す', '列の名前を変える', '列を消す', '並べ替え', '切り出す', '写す'];
  function 手順を読む(text) {
    ...
    for (var i = 0; i < 生.length; i++) {
      ...
      if (t.種類 === '写す' && (!範囲を読む(t.元) || !範囲を読む(t.先))) {
        断った.push('写す（どこから どこへか が 読めない）'); continue;
      }
      出.push(t);
    }
  ...
  function 手順を当てる(sheet, 手順, opt) {
    ...
    for (var i = 0; i < 手順.length; i++) {
      ...
      if (t.種類 === '式の列を足す') {
        ...
        /* ★同じシートの中で 写す★（マクロで よく出る書き方）
             例）Range("A1:D9").Copy Destination:=Range("F1")
                 Range("F1:I9").Value = Range("A1:D9").Value
           ★別のシートへ 写すのは ここでは しない★＝取り出す側で 断る
             （勝手に 同じシートへ 当てると ★別の所を 書き換える★事になる）。
           ★値だけ★の時は 式を 値に する（.Value = .Value と 同じ）。
           ★丸ごと★の時は 式を そのまま 運ぶ（.Copy と 同じ）。 */
        var 元範 = 範囲を読む(t.元), 先範 = 範囲を読む(t.先);
        if (!元範 || !先範) continue;
        var 写した = 0;
        for (var wr = 0; wr <= 元範.終行 - 元範.始行; wr++) {
          for (var wc = 0; wc <= 元範.終列 - 元範.始列; wc++) {
            /* ★1つ前の手順の 結果を 写す★＝手順は 上から 順に 起きる。
               ★実機で 押して 見つけた（2026-08-28）★＝元の表だけを 見ていたので、
                 並べ替えの ★後★に 写しても ★並べ替える前の 並び★を 写していた。
                 （Excelの マクロは 並べ替えた後の 物を 写す） */
            var 元番地 = (元範.始行 + wr) + ',' + (元範.始列 + wc);
            var から = Object.prototype.hasOwnProperty.call(変える, 元番地) ? 変える[元番地] : data[元番地];
            var さき = (先範.始行 + wr) + ',' + (先範.始列 + wc);
            if (t.値だけ) 変える[さき] = { v: から ? セルの値(から) : '' };
            else {
              変える[さき] = から
                ? (から.f ? { f: から.f } : { v: (から.v === undefined ? '' : から.v) })
                : { v: '' };
            }
            写した++;
          }
        }
        なに.push(t.元 + ' を ' + t.先 + ' へ 写す（' + 写した + 'か所'
          + (t.値だけ ? '・値だけ' : '') + '）');
      } else if (t.種類 === '切り出す') {
  ...
  /** ★A1 / A1:D9 を 行と列の番号にする★（0起点。読めなければ null）
   *  ★1つのセルだけの時は そこ1つ★（A1 → 1×1）。★行や列 まるごと（A:A・1:1）は 断る★
   *  ＝どこまで写すかが 決まらないまま 何万行も 書き換える事に なるため。 */
  function 範囲を読む(字) {
    var s = String(字 == null ? '' : 字).toUpperCase().replace(/\$/g, '').trim();
    var m = /^([A-Z]{1,3})(\d+)(?::([A-Z]{1,3})(\d+))?$/.exec(s);
    if (!m) return null;
    var c1 = 列の番号(m[1]), r1 = Number(m[2]) - 1;
    var c2 = m[3] ? 列の番号(m[3]) : c1, r2 = m[4] ? Number(m[4]) - 1 : r1;
    if (c1 < 0 || c2 < 0 || !(r1 >= 0) || !(r2 >= 0)) return null;
    return {
      始行: Math.min(r1, r2), 終行: Math.max(r1, r2),
      始列: Math.min(c1, c2), 終列: Math.max(c1, c2),
    };
  }
