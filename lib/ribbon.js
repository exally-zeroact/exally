...
(function (root, factory) {
  ...
  function グループのHTML(g, タブ名) {
    ...
    /* ★組の 右下の ↘（起動ツール）★ 2026-08-31
       実Excel は ほぼ 全部の 組に ↘ を 持つ（実測＝正本 26個）。
       ★26個の うち 25個は 開く先の 窓が うちに 既に 在る★ので
       ★新しい 窓は 作らない★＝★↘ を 描いて つなぐ だけ★。
       ★開く先が 無い ↘ は 出さない★（押しても 何も 起きない 印を 見せない）。
       中身の 正本：lib/ribbon-launcher.js ／ 見張り：tests/ribbon-launcher.test.mjs */
    var 開く印 = '';
    if (起動 && 出した) {
      var A = (typeof window !== 'undefined') ? window.RibbonActions : null;
      var 先 = 起動.引く(タブ名, g.name, A);
      if (先) {
        開く印 = '<button type="button" class="rb-launch" data-act="' + esc(先.先) + '"'
          + ' title="' + esc(先.Excel + '（' + 先.先 + '）') + '"'
          + ' aria-label="' + esc(先.先) + '">&#x2198;</button>';
      }
    }
