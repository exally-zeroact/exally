 ...
 *    ・YEN … ★実Excel に 存在しない★（2026-08-31 実測＝#NAME?）
  ...
  function 数える() {
    return {
      ...
      別のlibで足す: {
        ...
        'ENCODEURL': 'lib/encodeurl.js（実Excel に 75通り 打たせて 合わせた・外に 出ない）',
        'FILTERXML': 'lib/filterxml.js（実Excel に 29通り 打たせて 合わせた・外に 出ない）',
      },
      足さない: {
        ...
        /* ★FILTERXML は 2026-09-04 に 足した★＝★渡された 字を 読むだけ＝外に 出ない★
           （WEBSERVICE が 外へ 取りに 行く／FILTERXML は その 字を 読むだけ）lib/filterxml.js */
        /* ★ENCODEURL は 2026-09-04 に 足した★＝★外に 出ない（字を 変えるだけ）★
           ＝WEBSERVICE・RTD とは 別物。lib/encodeurl.js */
        'PHONETIC': 'ふりがなは セルに 付いた データ＝エンジンでは 見えない',
        'YEN': '★実Excel に 存在しない★（2026-08-31 実測＝#NAME?）。円は DOLLAR が 出す'
      }
    };
  }
