  ...
  function 数える() {
    return {
      ...
      別名で動く: {
        ...
        'ISREF': '★打てば そのまま 動く★＝参照かどうかを 見る。Exally が '
          + '式を 読んで TRUE()/FALSE() に 直して 答える（直している 場所＝convertFormula）',
      ...
      足さない: {
        ...
        'WEBSERVICE': 'ウェブの 住所から 字を 取ってくる 物。外へ 出る 口を まだ 開けていない',
        'RTD': '向こうから 押してくる 相手に 繋ぐ 物。繋ぐ 相手が 無い',
        'IMAGE': 'ウェブの 住所の 絵を セルに 出す 物。外へ 出る 口を まだ 開けていない',
        'PHONETIC': 'ふりがなは セルに 付いた データ＝エンジンでは 見えない',
        ...
        'AREAS': 'とびとびの 範囲（A1:B3,D1:D2 のように カンマで つないだ 物）を'
          + ' エンジンが まだ 受け取れない。1つの 範囲なら 数えられるが'
          + ' とびとびだと 答えが 出ない ⇒ 半分 合う 物は 出さないと 決めた（2026-09-07 実測）',
        'CELL': '行・列・番地は 出せるが、幅・書式・保護は'
          + ' ★画面の 見た目や ブックの 設定★を 見に 行く 物で、計算の 所からは 見えない。'
          + ' 一部だけ 返すと どれが 本物か 分からない ので 出していない（2026-09-07 実測）',
        'FILTERXML': 'XML から XPath で 取り出す 物。'
          + ' 取り出す 決まりの 書き方が 広く、少しだけ 作ると'
          + ' ★通る 式と 通らない 式が 混ざる★ ので 出していない（2026-09-07 実測）',
        'INFO': '「今の フォルダ」「使っている OS」など'
          + ' ★パソコンの 中の 事★を 返す 物。ブラウザの 中には その 物が 無い。'
          + ' 何を 返すかを 決めてから 出す（2026-09-07 実測）',
        ...
        'TRANSLATE': '外の 翻訳の 会社に 繋ぐ 物。契約が 要る',
        'DETECTLANGUAGE': '何語で 書かれているかを 見る 物。外の 会社に 繋ぐ 契約が 要る'
      }
    };
  }
