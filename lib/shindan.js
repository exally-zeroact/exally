...
(function (root) {
  ...
  var 言い方 = {
    ...
    thisrow_other_table: {
      題: 'ほかの表の「その行」を 見ています',
      なに: function (n) {
        return 'その行（[@列名]）で ★自分の表の 外★を 見ている式が ' + n + 'か所 あります。';
      },
      なぜ: '「その行」は 自分の表の中でしか 決まりません。'
        + 'Excel も 答えを出せず、IFERROR で包まれていると ★0 のまま★になります（数字を入れても 変わりません）。',
      つぎ: '直し方は1つです。式の中の 表の名前を ★自分の表の名前★に変えてください（★列名はそのまま★）。'
        + '下に 1か所ずつ「前 → 後」を出します。',
    },
  };
  /** ★直し方の 1行★（前 → 後）。★書き換えは しない。言うだけ★ */
  function 直し方の字(x) {
    if (!x || !x.直し方) return '';
    return x.直し方.前 + '[@' + (x.列名 || '…') + ']　→　' + x.直し方.後 + '[@' + (x.列名 || '…') + ']';
  }
  function 言葉(種類, n) {
    var w = 言い方[種類];
    if (!w) return null;
