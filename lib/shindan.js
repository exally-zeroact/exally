...
(function (root) {
  ...
  var 言い方 = {
    ...
    /* ★2本目（2026-09-04）★ 司さんの実物で 62か所 出た型。
       ★8月の 行を コピーして 9月を 作った時、表の 名前だけ 付いて来る★
       ⇒ 「その行」は 自分の表の 中でしか 意味を 持たないので どの行か 決まらない
       ⇒ ★Excel 自身も 答えを 出せない／IFERROR に 包まれていると 0 に なって 誰も 気づかない★
       ★ここは 直し方まで 言う★＝表の 名前を 1つ 変えるだけ（★列名は 触らない★）。 */
    /* ★★2026-09-04 に 書き直した（司さん「分かりやすく 説明しとんか？」）★★
       ★前の 文には 中の言葉が 入っていた★ … 「[@列名]」「IFERROR」
       ⇒★客は 中の言葉を 知りません／知る必要も ありません★（会社の 決まり）
       ⇒★しかも どこを どう 押して 直すかが 書いていなかった★
       ★今の 文の 決まり★
         ①中の言葉を 出さない（[@列名]／IFERROR／その行／band …）
         ②★何が 困るか★を 先に 言う（★ずっと 0 のまま★）
         ③★どこを 押して 何を 何に 変えるか★まで 言う（数式バー・1か所 直したら 下へ コピー） */
    thisrow_other_table: {
      題: 'ここは ずっと 0 のままです',
      なに: function (n) {
        return n + 'か所 あります。その式は 別の表の「同じ行」を 見ています。';
      },
      なぜ: '別の表の「同じ行」は どの行か 決まりません。だから Excel でも 計算できません。'
        + 'それでも エラーは 出ずに 0 が出ますので、数字を 入れても ずっと 0 のままです。',
      つぎ: '直し方 … そのセルを押して、上の式の欄で 表の名前だけを 書き替えてください'
        + '（そのうしろは そのまま）。1か所 直したら 下へコピーできます。'
        + '下の一覧を押すと その場所へ行きます。',
    },
    /* ★同じ「ずっと 0」だが 直し方が 違う★（2026-09-04）
       ＝そのセルが 表の 外に 在る（または 表の 合計の行）ので、
         「同じ行」が どの行か 決まらない。★変える 名前が 無い★＝移すか 行を 書く。 */
    thisrow_outside_table: {
      題: 'ここは ずっと 0 のままです',
      なに: function (n) {
        return n + 'か所 あります。その式は 表の「同じ行」を 見ていますが、'
          + 'そのセルは その表の 外に あります。';
      },
      なぜ: '表の外では「同じ行」が どの行か 決まりません。だから Excel でも 計算できません。'
        + 'それでも エラーは 出ずに 0 が出ますので、数字を 入れても ずっと 0 のままです。',
      つぎ: '直し方 … その式を 表の中へ 移すか、見たい行を はっきり書いてください。'
        + '下の一覧を押すと その場所へ行きます。',
    },
  };
  /** ★直し方の 1行★＝★何を 何に 変えるか だけ★（★書き換えは しない。言うだけ★）
   *  ★[@…] の 形は 出さない★＝客の 画面には 出ない 書き方（中の言葉）。
   *  ★どの列の 所かは 列の名前で 言う★（客は 列の名前なら 分かる）。 */
  function 直し方の字(x) {
    if (!x) return '';
    if (!x.直し方) {
      /* ★変える 名前が 無い 型★＝表の外から「同じ行」を 見ている */
      return (x.指した表 || '表') + ' の「同じ行」を 表の外から 見ている'
        + (x.列名 ? ('（' + x.列名 + ' の 所）') : '');
    }
    return x.直し方.前 + ' を ' + x.直し方.後 + ' に 変える'
      + (x.列名 ? ('（' + x.列名 + ' の 所）') : '');
  }
  function 言葉(種類, n) {
    var w = 言い方[種類];
