...
(function (root, factory) {
  ...
  /* ★溢れで 出来た マスの 印★＝book.html の `溢れの印` と ★同じ字★。
       （借り物の 溢れ・台の 溢れ の どちらでも この印が 付きます） */
  var SPILL_KEY = '_溢れ元';
  ...
  function gridToBook(sheets) {
    return {
      sheets: (sheets || []).map(function (sh) {
        ...
        /* ★★溢れ(スピル)の 範囲を 先に 数える★★（2026-09-20）
             ★なぜ 要るか★
               溢れた 式は 頭の マスに だけ 式が 在り、先の マスは 値です。
               今までは 頭を ★普通の 1マスの 式★として 書いて いたので
               実Excel で 開くと 隣に 値が 在る ⇒★#SPILL!★ に なります（実測）。
             ★何を 足すか★（SheetJS に 口が 在る事を 実物で 数えた・2026-09-20）
               ・`F` ＝ `<f t="array" ref="A1:A3">` を 書かせる（頭の マスに だけ）
               ・`D` ＝ `<c cm="1">` を 書かせる＝★動く並び(dynamic array)の 印★
                 （`xl/metadata.xml` は SheetJS が いつも 書いて います＝904B・
                   `dynamicArrayProperties fDynamic="1"` 在り・Content_Types と rels にも 登録済み）
             ★範囲の 出し方★
               溢れの 先の マスだけが `_溢れ元` に 頭の 'r,c' を 持って います
               （頭には 付きません＝`_溢れを写す` が i===0&&j===0 を 飛ばす為）
               ⇒ 先を 集めて ★頭ごとの いちばん 右下★を 求めます。 */
        var 溢れ範囲 = {};
        Object.keys(sh.data || {}).forEach(function (key) {
          var c0 = sh.data[key] || {};
          var 元 = c0[SPILL_KEY];
          if (typeof 元 !== 'string' || !元) return;
          var q = key.split(','), qr = parseInt(q[0], 10), qc = parseInt(q[1], 10);
          if (isNaN(qr) || isNaN(qc)) return;
          var g = 元.split(','), hr = parseInt(g[0], 10), hc = parseInt(g[1], 10);
          if (isNaN(hr) || isNaN(hc)) return;
          var t = 溢れ範囲[元];
          if (!t) t = 溢れ範囲[元] = { r: hr, c: hc, maxR: hr, maxC: hc };
          if (qr > t.maxR) t.maxR = qr;
          if (qc > t.maxC) t.maxC = qc;
        });
        Object.keys(sh.data || {}).forEach(function (key) {
          ...
          if (isF) {
            ...
            /* ★溢れの 頭なら 範囲と 動く並びの 印を 付ける★
                 SheetJS は `e.F` が ★自分の 住所で 始まる時だけ★ `t="array" ref=` を
                 書きます ⇒ 頭に だけ 立てる（先の マスは 値の まま＝実Excel と 同じ形）。 */
            var 範 = 溢れ範囲[key];
            if (範) {
              out.F = addr(範.r, 範.c) + ':' + addr(範.maxR, 範.maxC);
              out.D = true;
            } else if (String(cell.d || '') === '#SPILL!') {
              /* ★★塞がれた 溢れ★★＝溢れ先が 1マスも 無い（印が 付いた マスが 0）
                   ★付けないと どう なるか★（2026-09-20 実Excel で 実測）
                     `cm` も `ref` も 無い ⇒ ★昔の 式★として 読まれ
                     ⇒★暗黙の 交差で 先頭だけ★ 返り ★D1 が 黙って 1★（ISNUMBER True）
                     ⇒★誤りを 出すべき 所で 数を 出す＝一番 悪い 形★
                     ⇒塞ぎを 消しても ★溢れ直しません★
                   ★自分 1マスの 動く並び★として 書けば
                     実Excel が 計算し直して ★自分で #SPILL! を 出します★
                   ★なぜ `ref` が 自分 1マス（D1:D1）か★
                     ㅁグリッドに ★本来の 広さが 残って いません★
                       （塞がれた 時は `_溢れ元` が 1つも 付かない）
                     ㅂ`ref="D1:D3"` は ★中身が 食い違います★
                       D2 は ★値を 持つ マス★なのに ★配列の 一部★にも なる */
              out.F = addr(r, c) + ':' + addr(r, c);
              out.D = true;
            }
          } else {
  ...
  function exportWarnings(sheets, opts) {
    ...
    if (arrayCells.length) {
      out.push({
        ...
        /* ★★2026-09-10 に 字を 直しました（★半分だけ 本当に なって いた★）★★
           ★溢れ（1つの 式が 何マスにも 広がる）を 画面に 出すように した★ので
           書き出す 中身が 変わりました。★実Excel で 開いて 確かめて います★:
             前 … C1 に 式だけ ／ ★C2〜C5 は 空★
             後 … C1 に 式 ／ ★C2〜C5 に 2,3,4,5 が 入る★（ただの 数として）
           ⇒★値は 渡ります★（前は 消えて いました）＝★前より 良く なって います★
           ⇒ ただし ★Excel の 側で 溢れ直しは しません★（ただの 数なので）
           ⇒ 前の 字「1セル分の値になります」は ★式の 事だけ★で
             ★残りの 値も 渡る★事を 言って いませんでした＝★半分だけ 本当★ */
        msg: '配列を返す式が' + arrayCells.length + '件あります。'
          + 'Excelでは【式は1セル分】になり、【広がった先の値は ただの数として入ります】'
          + '（Excel側で widening し直しはしません）: ' + listCells(arrayCells)
      });
    }
    return out;
  }
