...
(function (root) {
  ...
  /** ★選んだ 値だけ 残す★（見出しの ▼ から 使う。byValue は 1つだけ・こちらは いくつでも）
   *  @param 残す … { 値の字: true } の 形
   *  ★見出し行は 隠さない★（実測: 1行目は False のまま）＝byValue と 同じ 決まり */
  function byValues(get, rng, 見出しあり, keyCol, 残す) {
    var 先頭 = rng.r1 + (見出しあり ? 1 : 0);
    var hide = [], keep = [];
    for (var r = 先頭; r <= rng.r2; r++) {
      if (残す[text(get(r, keyCol))]) keep.push(r); else hide.push(r);
    }
    return { hide: hide, keep: keep };
  }

  var api = { byValue: byValue, byValues: byValues, values: values, text: text };
