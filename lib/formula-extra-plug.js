 ...
 *    ARRAYTOTEXT / MODE.MULT / ★BAHTTEXT・ENCODEURL・FILTERXML（2026-09-04 足した）★
 *
...
(function (root) {
  ...
  function つなぐ(H, F) {
    ...
    var Plug = class extends H.FunctionPlugin {
      ...
      filterxml(ast, state) {
        /* ★中身は lib/filterxml.js★／★外に 出ない（渡された 字を 読むだけ）★
           ★答えは 実Excel に 29通り 打たせた 物★（tests/fixtures/filterxml-golden.json）
           ★こぼれる（縦1列）★＝範囲で 返す（形を 保つ） */
        var xml = this.evaluateAst(ast.args[0], state);
        if (xml instanceof CellError) return xml;
        var xp = this.evaluateAst(ast.args[1], state);
        if (xp instanceof CellError) return xp;
        var FX = (typeof self !== 'undefined' && self.FilterXml) ? self.FilterXml
          : ((typeof require === 'function' && typeof module === 'object') ? require('./filterxml.js') : null);
        if (!FX) return new CellError(ErrorType.VALUE);
        var 空 = function (v) {
          return (v === null || v === undefined || typeof v === 'symbol'
            || (H.EmptyValue !== undefined && v === H.EmptyValue));
        };
        var r = FX.取り出す(空(xml) ? '' : xml, 空(xp) ? '' : xp,
          (typeof self !== 'undefined' ? self : undefined));
        if (r && r.誤り) return new CellError(ErrorType[r.誤り] || ErrorType.VALUE);
        /* ★中の #VALUE! も エンジンの 形に 直す★（1つだけ エラーに なる 型が 在る） */
        var 表 = r.map(function (row) {
          return row.map(function (v) {
            return (v && typeof v === 'object' && v.誤り)
              ? new CellError(ErrorType[v.誤り] || ErrorType.VALUE) : v;
          });
        });
        return 範囲に(表);
      }
    ...
    Plug.implementedFunctions = {
      ...
      'FILTERXML':   { method: 'filterxml',   parameters: [何でも, 何でも] }
    };
