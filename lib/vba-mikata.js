...
(function (root) {
  ...
  var 分け方 = [
    ...
    { key: 'erabu', 印: '.Select / Selection', 名: '選んでから作業', 可否: 'かえる',
      手掛かり: [/\.Select\b/i, /\.Activate\b/i, /\bSelection\b/i],
      何を: 'セルを 選んでから 作業しています。', うち: '選ばずに 場所を指して 直す（要らない手順）' },
    { key: 'chiratsuki', 印: 'ScreenUpdating', 名: 'ちらつき止め', 可否: 'かえる',
    ...
    { key: 'keisan', 印: '.Calculate', 名: '計算のやり直し', 可否: 'かえる',
  ...
  function 見立てる1本(手続き) {
    ...
    for (var i = 0; i < 分け方.length; i++) {
      ...
      if (n) 当たり.push({ key: d.key, 名: d.名, 可否: d.可否, 何を: d.何を, うち: d.うち, 数: n, 手掛かり: d.印 || d.key, 先に見る: !!d.先に見る });
    }
    当たり.sort(function (a, b) {
      if (a.先に見る !== b.先に見る) return a.先に見る ? -1 : 1;   /* ★きっかけが 先★ */
      return b.数 - a.数;
    });
    /* ★分からない物は「分からない」と言う★＝当てずっぽうで 分類しない */
    if (!当たり.length) {
