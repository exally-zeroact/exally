...
(function (root) {
  ...
  /**
   * ★2つ以上の 鍵で 並べ替える★（Excelの「並べ替え…」）2026-08-29
   *   ★真値（実Excel 16.0 で 実測）★ 点↑ → 名前↑ で 並べると:
   *     見出し / ようかん10 / あんこ20 / ★ういろう30 / かすてら30★
   *     ⇒ ★同じ点は 2つ目の鍵（名前）で 決まる★／★見出しの行は 動かない★
   *   @param 鍵たち [{cells:[...], dir:'asc'|'desc'}, …]（前の物ほど 強い）
   *   @returns 並べ替えた 後の 元の位置の 並び
   */
  function order2(鍵たち) {
    if (!鍵たち || !鍵たち.length) return [];
    var n = 鍵たち[0].cells.length;
    var 印 = [];
    for (var i = 0; i < n; i++) {
      var k = [];
      for (var j = 0; j < 鍵たち.length; j++) k.push(型と値(鍵たち[j].cells[i]));
      印.push({ i: i, k: k });
    }
    /* ★1つ目の鍵が 空の物は 常に 最後★（1つの鍵の 時と 同じ決まり） */
    var 中身 = 印.filter(function (x) { return x.k[0].t !== 空; });
    var 空の物 = 印.filter(function (x) { return x.k[0].t === 空; });
    中身.sort(function (a, b) {
      for (var m = 0; m < 鍵たち.length; m++) {
        var d = くらべる(a.k[m], b.k[m]);
        if (d !== 0) return 鍵たち[m].dir === 'desc' ? -d : d;
      }
      return a.i - b.i;                                    /* 全部 同じなら 元の順 */
    });
    return 中身.map(function (x) { return x.i; }).concat(空の物.map(function (x) { return x.i; }));
  }
  /**
  ...
  var api = { order: order, order2: order2, region: region, guessHeader: guessHeader, hasAbsRef: hasAbsRef, 型と値: 型と値 };
