...
(function (root) {
  ...
  function 作る(sheets, 網, 診断, opt) {
    ...
    /* ★★機械が 見つけた 物は 1つ残らず AIへ 渡す★★（2026-09-04）
       ★正本（2026-08-22）の 核心＝「気づくのは 機械／AIは 説明するだけ」★
       ⇒★見つけたのに 渡さない＝持っているのに 使っていない★（今日 2本目で 実際に そうなっていた） */
    if (opt.表の断り && opt.表の断り.length) {
      行.push('');
      行.push('## 機械が見つけた 危ない所（AIは呼んでいない）その2');
      行.push('- ほかの表の「その行」を見ている式 … ' + opt.表の断り.length + 'か所'
        + '（自分の表の外は 行が決まらない＝Excelも答えを出せない。IFERRORで 0のままになる）');
      var 直し = opt.表の断り.slice(0, 5).map(function (x) {
        if (!x.直し方) return x.シート + '!' + x.セル + ' ' + x.指した表 + '[@' + (x.列名 || '')
          + '] を 表の外から見ている';
        return x.シート + '!' + x.セル + ' ' + x.直し方.前 + '[@' + (x.列名 || '') + ']→'
          + x.直し方.後 + '[@' + (x.列名 || '') + ']';
      });
      行.push('- 例: ' + 直し.join(' / ')
        + (opt.表の断り.length > 5 ? (' ほか ' + (opt.表の断り.length - 5) + 'か所') : ''));
      行.push('- 直し方: 式の中の 表の名前だけを 自分の表の名前に変える（うしろはそのまま）。'
        + '表の外から見ている物は 表の中へ移すか 行をはっきり書く');
    }
    if (診断 && 診断.式の本数) {
      行.push('');
