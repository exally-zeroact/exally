...
(function (root, factory) {
  ...
  /* ══ ★★部品を 足す★★ ══（2026-09-21）
       ★★なぜ 要るか★★
         ...
         ⇒★足りないのは この 口 1つ★ でした。
       ★★なぜ 安全か★★
         `_assemble()` は ★全部の 部品を 頭から 作り直して います★
         （局所ヘッダ・中央目録・EOCD を その場で 組み立てる）
         ⇒★入れ物を 1つ 増やすだけ★＝★既存の 部品の 作り方を 1行も 変えません★
       ★★同じ 名前が 在る 時は 投げます★★
         ＝★黙って 上書きしない★（上書きは `replace` の 仕事）
         ＝★同じ 名前が 2つ 在る 包みは Excel が 修復を 言います★
       ★★測って いない 事★★
         ・★実Excel が この 包みを どう 開くかは ここでは 測れません★（COM が 要る）
         ・★足した 後の `[Content_Types].xml` や rels は 呼ぶ 側の 仕事★
           ＝★この 口は 包みに 入れるだけ★です */
  Zip.prototype.add = function (name, u8) {
    if (!name) throw new Error('部品の名前がありません');
    var 前 = this.index[name];
    if (前 && !前.removed) throw new Error('もう在る部品です（上書きは replace）: ' + name);
    var 体 = (u8 instanceof Uint8Array) ? u8 : toBytes(String(u8));
    if (前) {
      /* ★1度 消した 名前を 足し直す 時は その 入れ物を 使い回します★ */
      前.removed = false; 前.replaced = 体; 前.packed = null; 前.packedMethod = 0;
      return this;
    }
    /* ★★時刻は 元の 包みの 1本目に 合わせます★★
         ＝★今の 時刻を 入れると 同じ 材料から 違う 包みが 出ます★
         ＝記憶「試験に 今日が 何月かを 持ち込むな」と 同じ 形
         ＝1本も 無ければ 0（zip は 0 を 許します） */
    var 見本 = this.entries.length ? this.entries[0] : null;
    var e = {
      name: name, method: 0, flag: 0,
      time: 見本 ? 見本.time : 0, date: 見本 ? 見本.date : 0,
      crc: 0, csize: 0, usize: 0,
      raw: null, replaced: 体, removed: false,
    };
    this.entries.push(e);
    this.index[name] = e;
    return this;
  };
  Zip.prototype.addText = function (name, s) { return this.add(name, toBytes(s)); };
