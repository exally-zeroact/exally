...
(function (root, factory) {
  ...
  /** ★1つに まとめる★
   *  ★★名を `数える` に しては いけません★★（2026-09-21 に 踏みました）
   *    `lib/hairanai.js` の 頭に ★同じ 断りが 先に 書いて あります★。
   *    `lib/*.js` `js/*.js` に `function 数える(` は ★11個★ 在り、
   *    `tests/unused-param.test.mjs` は ★名前で★ 呼び出しを 数えるので、
   *    ここに `数える(本)` を 足すと ★その 11個と 混ざって 門が 赤に なります★。
   *    ⇒★読んだ のに 踏みました★＝記憶「紙は 書いた だけでは 効かない」そのもの。
   */
  function 本の中身を数える(本) {
    var b = 本 || {};
    return {
