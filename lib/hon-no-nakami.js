...
(function (root, factory) {
  ...
  function マクロを数える(読み, 見立て) {
    ...
    return {
      ありさま: '在ります',
      本数: (読み.モジュール || []).length,   /* ★かたまり（モジュール）の 数★ */
      なぜ: String(読み.なぜ || ''),
      手続き: 手,
      /* ★★「見立てを 渡されたか」と 「手続きが 0本か」は 別★★
           ＝渡されて いないのに 0本と 書くと ★「手続きが 無い」と 読まれます★ */
      見立てた: !!見立て,
      手続きの本数: (見立て && 見立て.本数) || 0,
    };
  }
  ...
  function 一文(数え) {
    ...
    if (m.ありさま === '無し') 出.push('マクロは ありません');
      ...
      /* ★★2026-09-22 ── ★同じ 窓に 4 と 5 が 並んで いました★★（Exally1 が 見つけました）
           1文   ... `読み.モジュール` の 数 ＝ ★かたまり（モジュール）★
           詳しく ... `見立て.手続き` の 数   ＝ ★手続き（Sub / Function）★
         ⇒★どちらも 正しい 数★です。★言葉が 揃って いなかった★だけ。
         ⇒★お客さんから 見れば「どっちが 本当か」になります★
         ⇒★両方 書きます★（★片方を 消すと 数が 減ったように 見えます★）
         ★見立てを 渡されて いない 時は 手続きの 数を 書きません★
           ＝★0本と 書くと「手続きが 無い」に 読めます★（未測定と 0は 別） */
      var 文 = 'マクロが ' + m.本数 + 'かたまり';
      if (m.見立てた) 文 += '（手続き ' + m.手続きの本数 + '本）';
      出.push(文);
    }
    return 出.join('／') + '。';
  }
  ...
  function 詳しく(数え) {
    ...
    var m = d.マクロ || {};
    if (m.ありさま === '在ります') {
      /* ★★見立てを 渡されて いない 時に 「0件」と 言い切らない★★（2026-09-22）
           ＝★手続きが 無い★ と ★手続きを 数えて いない★ は 別です
           ＝`0件` と 出すと お客さんは「マクロは 空」と 読みます */
      出.push({
        /* ★1文の 「かたまり」と 揃えます★＝★ここは 手続きの 数★ */
        見出し: m.見立てた ? 'マクロの 手続き' : 'マクロの 手続き（数えていません）',
        ...
        注: m.見立てた
          ? (m.なぜ ? ('読んだ 時の 断り ... ' + m.なぜ) : '')
          : ('マクロは ' + m.本数 + 'かたまり 在りますが、★手続きに 切って いません★'
            + '（見立てを 渡されていません）。'
            + (m.なぜ ? '読んだ 時の 断り ... ' + m.なぜ : '')),
      });
