...
- ISREF … ★マスを 指しているか★を 見る。★A1／A1:B2／A:A／1:1／Sheet1!A1 は そのまま 動く★（実Excel と 突き合わせ済み）。★中に 関数を 入れた 形（=ISREF(INDEX(...)) など）は まだ★＝#NAME? に なる。★指せたか どうかが 字だけでは 決まらない為（INDIRECT("A1") は TRUE／INDIRECT("zzz") は FALSE）。間違った TRUE/FALSE を 出すより「まだ」と 言う
