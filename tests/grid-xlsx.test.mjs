...
T('⑧ ★配列を返す式は件数と番地を伝える', () => {
  ...
  /* ★★2026-09-10 に 言い方を 直しました（★半分だけ 本当だった★）★★
     溢れ（1つの 式が 何マスにも 広がる）を 画面に 出すように した ので
     書き出す 中身が 変わりました。★実Excel で 開いて 確かめて います★:
       前 … C1 に 式だけ ／ ★C2〜C5 は 空★
       後 … C1 に 式 ／ ★C2〜C5 に 2,3,4,5 が ただの 数で 入る★
     ⇒ 前の 字は ★式の 事だけ★を 言って いて ★残りの 値も 渡る★事を
       言って いませんでした＝★お客さんは「値は 消える」と 読む★
     ★ここは 字を そのまま 持って います★＝言い方を 変えたら ★ここも 直す★
     （★言い方が 勝手に 変わって いない事★を 見る のが この 試験の 役目） */
  if (!/配列を返す式が1件あります。/.test(a.msg)) throw new Error('言い方: ' + a.msg);
  if (!/式は1セル分/.test(a.msg)) throw new Error('★式が 1セル分に なる 事を 言って いない★: ' + a.msg);
  if (!/ただの数として入ります/.test(a.msg)) throw new Error('★広がった先の 値も 入る 事を 言って いない★: ' + a.msg);
  eq(a.cells, ['請求!A7']);
});
