...
const まだ = new Map([
  ...
  ["=BINOM.DIST.RANGE(200,0.05,25)",
    "★実Excel は 組み合わせを きっちり 出して いない＝`=BINOM.DIST.RANGE(10,0.5,3)` が ★0.11718750000000003★（きっちりは 0.1171875）＝★足し方の 違い＝15桁目★（棚58）★"],
  ["=AVERAGE(\"1900-01-01\")",
    "★字を 数に する 時に 日付・時刻を 読んで いない（「1900-01-01」→1 ／ 「12:3」→0.502…）＝★土台の 話★（棚59）"],
  ["=AVERAGE(\"1900-02-29\")",
    "★字を 数に する 時に 日付・時刻を 読んで いない（「1900-01-01」→1 ／ 「12:3」→0.502…）＝★土台の 話★（棚59）"],
  ["=AVERAGE(\"12:3\")",
    "★字を 数に する 時に 日付・時刻を 読んで いない（「1900-01-01」→1 ／ 「12:3」→0.502…）＝★土台の 話★（棚59）"],
  ...
  ["=AVERAGE(\"1/2\")",
    "★年が 抜けた 日付は ★今年★で 読む＝紙は 2026年で 取った＝★今日に よって 変わる★（棚59）"],
  ["=AVERAGE(\"0000-01-01\")",
    "★0000年は 日に ならない＝実Excel は #VALUE!（うちは 3 を 返す）＝★土台の 話★（棚59）"],
]);
...
for (const p of 紙たち) {
  ...
  for (const l of 行たち) {
    ...
    const 名 = (式列 > 0 ? String(c[0] || '') : 式.replace(/^=\s*/, '').split(/[^A-Za-z0-9_.]/)[0]).trim().toUpperCase();
