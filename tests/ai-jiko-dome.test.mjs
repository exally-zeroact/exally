...
if (SELF) {
  ...
  const BREAKS = [
    /* ★2026-09-05★ 数は lib/ai-genkai.js の 1か所に 集めた（掘る数と 噛み合っていなかった）。
       ⇒ ここで 壊すのは ★正本の 方★。api/claude.js の べた書きは もう 無い。 */
    /* ★2026-09-05★ 数は lib/ai-genkai.js の 1か所に 集めた（掘る数と 噛み合っていなかった）。
       ★ここで 壊すのは「api/claude.js で べた書きに 戻す」形★＝
       この 仕掛けは api/claude.js しか 差し替えられない（OVERRIDE）ので、
       ★lib 側の 数そのものは 上の `eq(handler.__事故止め.分の回数, 10)` が 直に 見張っている★
       （lib を 変えれば その 検査が 落ちる＝実測で 確かめた）。 */
    ['api/claude.js', '★1分の上限を 外す（べた書きに 戻す）★',
      (s) => s.replace('分の回数: AI限界.分の回数,', '分の回数: 100000,')],
