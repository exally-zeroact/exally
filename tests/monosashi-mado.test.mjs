...
if (自己試験) {
  ...
  T('★免除に 逃げて いない（免除は 1本だけ）★', () => {
    /* ★★上限を 3 → 4 に 上げました★★（2026-09-15・★数字を 黙って 動かさない★）
       ★足したのは 1本★＝`docs/measured/toru-jitsubutsu-shoshiki.ps1`
       ★訳★＝★`.Value2` を 1度も 呼びません★（司さんの 実物から ★書式の 字と 本数だけ★ 数える 道具）
         ＝★「0」が 出る 所が 無い★＝2つ目の 窓の 当たる 先が ありません。
       ★同じ 日に もう 1本 作りましたが 免除して いません★
         （`toru-shoshiki-dai.ps1`＝★窓②を 足しました★。
           ★「生の 数だから 要らない はず」で 逃げず 測った★ら
           ★見た目 0・中は 0.004 の 行が 6本★ 出ました＝★足して 正解でした★）
       ⇒★上限は「逃げ道の 数」では なく「逃げた 回数」を 見る 為の 物★＝
         ★1本 足すたび ここに 訳を 書く★ */
    if (Object.keys(免除).length > 4) {
