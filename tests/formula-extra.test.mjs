...
/* ★数を 決め打ちに しない★＝★どれも「どこに 在るか」が 書いてある事★を 見る
   （2026-09-04 … BAHTTEXT の 次に ENCODEURL を 足したら 1個の 決め打ちで 赤に なった） */
const 別 = Object.keys(数.別のlibで足す);
言う(別.length >= 1, '★別の lib で 足した 物が 在る（今 ' + 別.length + '個）★');
for (const k of 別) {
  言う(/lib\/[a-z-]+\.js/.test(数.別のlibで足す[k]),
    '★' + k + ' … どこに 在るかが 書いてある★', 数.別のlibで足す[k]);
  言う(!数.足さない[k], '★足さない 一覧から ' + k + ' が 消えている（もう 足した）★');
}
