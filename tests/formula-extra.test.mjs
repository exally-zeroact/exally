...
/* ★★2026-09-05 実測で 直した（08-31 の 決めつけが 試験に 焼いてあった）★★
   ...
   ⇒★足さないのは 変わらない★が、理由が 嘘だった
   ★見るのは 字ではなく「お客さんが 次に どうなるか」★
     ①代わりに 何を 使うかが 書いてある（DOLLAR）
     ②★打った YEN が そのまま 動く事★＝どこが 直すかが 書いてある
   ★本当に 動くかは text-format.test.mjs で ★engine に 通して★ 測る★
     （ここは 台帳の 字だけ＝それだけでは 半分） */
言う(数.足す.indexOf('YEN') < 0,
  '★YEN は 足さない（本名 DOLLAR に 直して 動かす＝別名を 増やさない）★');
言う(!!数.足さない.YEN && /DOLLAR/.test(数.足さない.YEN),
  '★YEN の 理由に「代わりに 何が 本名か」が 書いてある★', 数.足さない.YEN);
言う(/convertFormula/.test(String(数.足さない.YEN || '')),
  '★YEN の 理由に「どこが 直すか」が 書いてある（打った人は そのまま 動く）★', 数.足さない.YEN);
const 理由なし = Object.keys(数.足さない).filter((k) => !String(数.足さない[k]).trim());
言う(理由なし.length === 0, '★足さない 物には 全部 理由が 書いてある★', 理由なし.join(' / '));
