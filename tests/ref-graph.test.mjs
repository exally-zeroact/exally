...
for (const b of 実物) {
  ...
  T('★実物 ' + b.名 + '：別シートを見ている式 ' + b.式の本数 + '本を 1本も落とさない★', () => {
    ...
    /* ★★2026-09-04 に 変えた（司さんの 指摘）★★
       ★前★ … シート数・式の数・のべ本数・式の本数を ★台帳の 数と 同じか★ で 見ていた
       ★何が 起きたか★ … 司さんが 実物を 触ると（1日に 3回 保存した）★毎回 赤★
                          ＝★中身が 良くなった 時でも 赤★＝★見張りの 作りが 悪い★
       ★今★ … ★数は 記録として 出す。判定は「壊れたら 必ず 崩れる 辻褄」だけ★
                ①★取り違えが 0★（名前を 間違えたら ここが 増える）
                ②★のべ ≧ 式の本数★（1本の式が 何か所 見ても のべの方が 多い）
                ③★空振りしていない★（実物なのに 0本 は おかしい）
       ⇒★司さんが 触っても 赤に ならない／読み取りが 壊れたら 必ず 赤★ */
    const のべ = g.別シート参照.length;
    const 式の本数 = new Set(g.別シート参照.map((r) => r.from + ':' + r.fromCell)).size;
    console.log('       … 実測 ' + g.シート数 + 'シート／式 ' + g.式の数.toLocaleString() + '本'
      + '／別シート のべ ' + のべ.toLocaleString() + '・式 ' + 式の本数.toLocaleString()
      + '（台帳を 書いた日は ' + b.シート数 + 'シート／式 ' + b.式の数.toLocaleString()
      + '／のべ ' + b.のべ本数.toLocaleString() + '・式 ' + b.式の本数.toLocaleString() + '）');
    eq(g.分からない参照.length, 0, '★名前を取り違えている★');
    ok(のべ >= 式の本数, '★のべが 式の本数より 少ない＝数え方が 壊れている★');
    ok(式の本数 > 0, '★実物なのに 別シート参照が 0本＝空振りしている★');
    ok(g.式の数 > 0 && g.シート数 > 0, '★式もシートも 0＝読めていない★');
