...
if (!fs.existsSync(実物)) {
  ...
  T('★実物（15シート・式15,769本）の地図が 数千文字に収まる★', () => {
    ok(m.文字数 <= Chizu.既定の上限, '★' + m.文字数 + '文字★');
    ok(m.字.indexOf('15') > 0, 'シート数が入っていない');
    console.log('       … 実物の地図 ★' + m.文字数 + '文字★（ブック全体は 約142万文字）'
      + ' ／ 削った ' + m.削った + '文字');
  });
  T('★実物の15シートで 行×列が 指示役の実測と一致する（値か式／表の枠）★', () => {
    /* ★指示役が ws['!ref'] を decode_range して数えた値★（2026-08-27）
       ★2026-09-04 に 給料9 を 足し、計算を 404→405 にした（実物が 更新された）★ */
    const 指示役 = {
      '給料1': [82, 12], '給料2': [82, 12], '給料3': [82, 12], '給料4': [82, 12],
      '給料5': [82, 12], '給料6': [82, 12], '給料7': [82, 12], '給料8': [82, 12],
      /* ★2026-09-04 に 足した★＝司さんが 2026-09-03 19:36 に シートを 1枚 増やした */
      '給料9': [82, 12],
      /* ★計算は 404行→405行★（同じ 更新で 1行 増えた） */
      '給料表': [468, 131], '売上表': [39, 60], '計算': [405, 152],
    ...
    console.log('       … 15シートとも 表の枠が一致（例 計算 405行×152列・給料表 468行×131列）');
  });
