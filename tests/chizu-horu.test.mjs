...
T('★画面が 地図を毎回 渡す／掘る／止める／金額を差し替える★', () => {
  ...
  /* ★2026-09-04 に 印を 変えた★＝掘れる回数が ★5回の 決め打ち★から
     ★そのブックを 掘りきれる 回数★に なった（司さん「なんで できてないんど」）。
     ★ゆるめて いません★＝★止める 仕組みが 在る事★は そのまま 見る。 */
  ok(/Horu\.もう掘れないか\(掘った, \{ 掘れる回数: 掘れる \}\)/.test(book), '★掘りっぱなし（暴走する）★');
  ok(/Horu\.掘れる回数を決める\(sheets\)/.test(book), '★掘れる回数を ブックから 出していない（決め打ちに 戻った）★');
  ok(/回 < 掘れる \+ 2/.test(book), '★回す 回数が 決め打ちのまま★');
...
if (!fs.existsSync(実物)) {
  ...
  /* ★2026-09-04 に 数え直した★＝実物が 2026-09-03 19:36 に 更新された
     （★シート「給料9」が 1枚 増え、計算シートが 404行→405行★＝式 15,126→15,769）
     ⇒★減ったのでは なく 増えている＝実物が 増えたから★ */
  T('★実物の地図が 数千文字に収まる（シート数は 実測を 使う）★', () => {
    /* ★2026-09-04★ シート数を 決め打ちに していたので、司さんが 1枚 足すたび 赤に なった。
       ★今 読んだ シート数★を そのまま 使う（★数は 記録として 出す★）。 */
    ok(m.文字数 <= Chizu.既定の上限, '★' + m.文字数 + '文字★');
    ok(m.字.indexOf(String(sheets.length)) > 0, 'シート数が入っていない');
    console.log('       … 実物の地図 ★' + m.文字数 + '文字★（ブック全体は 約142万文字）'
      + ' ／ 削った ' + m.削った + '文字');
  });
  T('★実物の 表の枠が 中身より 小さくない（読み取りが 縮んでいない）★', () => {
    /* ★★2026-09-04 に 変えた（司さんの 指摘）★★
       ★前★ … 15シートの 行×列を ★指示役が 数えた 値そのもの★と 突き合わせていた
       ★何が 起きたか★ … 司さんが 実物を 触ると（シートを 1枚 足した・1行 足した）★毎回 赤★
                          ＝★中身が 増えた だけでも 赤★＝★見張りの 作りが 悪い★
       ★今★ … ★数は 記録★。判定は ★枠が 中身を 全部 入れているか★ だけ。
                ①どのシートも 枠が 取れている（未測定が 0）
                ②枠の 行×列 ≧ ★この試験が 自分で 数えた 中身の 端★
                ⇒★これが 崩れる時＝読み取りが 黙って 縮んだ時★（合計が 静かに 減る型）
       ★元の 指示役の 数（2026-08-27）★は 台帳として 下に 残す。 */
    const 元の実測 = {
    ...
    for (const s of sheets) {
      ...
      let 端行 = 0, 端列 = 0;
      for (const rc of Object.keys(s.data)) {
        const p = rc.split(','), r = +p[0], c = +p[1];
        if (r + 1 > 端行) 端行 = r + 1;
        if (c + 1 > 端列) 端列 = c + 1;
      }
      if (s.枠.行数 < 端行 || s.枠.列数 < 端列) {
        ちがい.push(s.name + ' … 枠 ' + s.枠.行数 + '行×' + s.枠.列数 + '列 なのに 中身は '
          + 端行 + '行×' + 端列 + '列（★枠が 中身より 小さい＝縮んでいる★）');
      }
      const 前 = 元の実測[s.name];
      記録.push(s.name + ' ' + s.枠.行数 + '×' + s.枠.列数
        + (前 ? ((前[0] === s.枠.行数 && 前[1] === s.枠.列数) ? '' : '（08-27は ' + 前[0] + '×' + 前[1] + '）') : '（08-27に 無い）'));
    }
    eq(ちがい.join(' / '), '', '★枠が 中身を 入れていない★');
    ok(sheets.length > 0, '★シートが 0枚＝読めていない★');
    console.log('       … ' + sheets.length + 'シート … ' + 記録.join(' ／ '));
  });
...
if (SELF) {
  ...
  const BREAKS = [
    ...
    ['book.html', '★掘りっぱなし（止めない）★',
      (s) => s.replace('if(Horu.もう掘れないか(掘った, { 掘れる回数: 掘れる })){', 'if(false){')],
    ['book.html', '★掘れる回数を 決め打ちに 戻す★',
      (s) => s.replace('Horu.掘れる回数を決める(sheets) : 5;', '5 : 5;')],
    ['book.html', '★回す 回数を 決め打ちに 戻す★',
      (s) => s.replace('回 < 掘れる + 2', '回 < 12')],
