...
if (!fs.existsSync(実物)) {
  ...
  /* ★2026-09-04 に 数え直した★＝実物が 2026-09-03 19:36 に 更新された
     （★シート「給料9」が 1枚 増え、計算シートが 404行→405行★＝式 +643本）
     前 … 式15,126／INDEX+MATCH 6,832／重複 2,548
     ⇒★減ったのでは なく 全部 増えている＝実物が 増えたから★ */
  T('★実物で 提案が出る（数は 記録・辻褄だけ 見る）★', () => {
    /* ★★2026-09-04 に 変えた（司さんの 指摘）★★
       ★前★ … 式の数・INDEX+MATCH・重複を ★決め打ちの 数★で 見ていた
       ★何が 起きたか★ … 司さんが 実物を 触ると ★毎回 赤★（1日に 3回 保存した）
       ★今★ … ★数は 記録★。判定は ★自分で 数え直した 物と 合うか★ だけ。
                ①見た式 ＝ ★シートの 中の 式の 数（この試験が 自分で 数えた物）★
                ②提案の 数 は 見た式 を 超えない
                ③空振りしていない（実物なのに 0）
       ⇒★司さんが 触っても 赤に ならない／数え方が 壊れたら 必ず 赤★ */
    let 式の数 = 0;
    for (const sh of sheets) for (const rc of Object.keys(sh.data)) {
      const f = sh.data[rc].f;
      if (typeof f === 'string' && f.charAt(0) === '=') 式の数++;
    }
    const im = r.提案.find((t) => t.種類 === 'indexmatch_to_xlookup');
    const ju = r.提案.find((t) => t.種類 === 'match_no_juufuku');
    console.log('       … 実測 式 ' + 式の数.toLocaleString() + '本／INDEX+MATCH '
      + (im ? im.何本.toLocaleString() : 0) + '／重複 ' + (ju ? ju.何本.toLocaleString() : 0));
    eq(r.見た式, 式の数, '★自分で 数え直した 式の数と 合わない★');
    ok(im && im.何本 > 0, '★INDEX+MATCH が 0＝空振りしている★');
    ok(ju && ju.何本 > 0, '★重複が 0＝空振りしている★');
    ok(im.何本 <= 式の数 && ju.何本 <= 式の数, '★提案の数が 式の数を 超えている★');
