 ...
 *    ＋㉘ `ACCRINT(DATE(...))` `#VALUE!`／16.666666666666664（★これは 直しません★）
 *
 *  ★★㉘（お金の `DATE()`）は この 紙では 直しません★★
 *    ＝因は ★借り物の 中（`NUMBER_DATE`）★＝★借り物外しと 同じ 山★
 *    ⇒★★だから その 1件は 「まだ」と 名指しで 許します★★（★黙って 見逃さない★）
 *
...
const まだ = new Map([
  ['kane DATEで 渡す(ACCRINT)', '★㉘＝因は 借り物の 中（NUMBER_DATE）／借り物外しと 同じ 山★'],
]);
