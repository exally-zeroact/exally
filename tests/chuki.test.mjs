...
/* ★正規表現の中の バッククォートで 迷子に ならない（2026-08-29 実際に踏んだ）★
   book.html に  text.replace(/`([^`]+)`/g, …)  が在る。
   ★バッククォートが 3つ（奇数）★なので、正規表現を 知らないと
   「ここから 字が 始まった」と 誤解し、★その先 800行ぶんの 注記を 落とし損ねる★。
   （後読みの見張りが「book.html に 後読みが 1件 残っている」と 嘘の赤を 出して 見つかった） */
T('★正規表現の中の バッククォートで 迷子に ならない★', () => {
  const B = String.fromCharCode(96);
  const NL = String.fromCharCode(10);
  /* ★実物と 同じ形に する★＝後ろにも バッククォートが 在る事。
     それが 無いと「閉じる相手が 居ない」で たまたま 助かってしまい、★見張りが 空振りする★
     （2026-08-29 実際に そうなった。book.html では 後ろに 別の ` が 在るので 落ちた）。 */
  const src = 'text = text.replace(/' + B + '([^' + B + ']+)' + B + '/g, "x");' + NL
    + '/* ここは 注記 AI_LIMIT */' + NL + 'var a = ' + B + 'あとの字' + B + ';';
  ok(!動く所に在る(src, 'AI_LIMIT'), '★正規表現の中の ` で 迷子に なり、後ろの注記を 落とし損ねた★');
});
T('★割り算の / を 正規表現と 読み違えない★', () => {
  const NL = String.fromCharCode(10);
  const src = 'var x = a / b; var y = c / d;' + NL + '/* 注記 AI_LIMIT */' + NL + 'var z = 1;';
  ok(!動く所に在る(src, 'AI_LIMIT'), '★割り算を 正規表現と 読んで 迷子に なった★');
  ok(動く所に在る(src, 'var y'), '★割り算の間を 消してしまった★');
});
T('★正規表現の中の 注記らしき字は 消さない★', () => {
  const src = 'var re = /https:\\/\\/x/g; var q = 1;';
  ok(動く所に在る(src, 'https'), '★正規表現の中の // を 注記として 消した★');
});
