/* kansuu46-9wakume.test.mjs — ★9枠目の 紙で 押す★（2026-09-18）
 *
 ...
 *    ★F1:G2 = 1,2／10,20★（★行ごと 3・30 ／ 列ごと 11・22＝分かれる★）
 *    ★式は J1★（★材料の 外★）
 *
 *  ★★押す 行の 選び方★★
 ...
 *
 *  使い方: node tests/kansuu46-9wakume.test.mjs
 */
...
console.log('');
console.log('[kansuu46-9wakume] ★9枠目の 紙で 押す★');

const 紙道 = path.join(ROOT, 'docs/measured/golden-kansuu-9kaime-2026-09-18.tsv');
...
const まだ = new Map([
  ...
  /* ══ ★ラムダの 一族が まだ★（2026-09-18）══
       ★外側（SUM／ROWS／LET）は 台が 知って います★＝★押して います★
       ★中の LAMBDA を 知りません★ ⇒ #NAME? か ★1マス扱い★に なります
       ⇒★★書いたら この 許しは 自動で 外れます★★
         （「合ったのに まだ の まま」で 赤に なる） */
  ['=SUM(BYROW(F1:G2,LAMBDA(r,SUM(r))))',
    '★ラムダの 一族が まだ★（段取り ⑵〜⑷）＝★外側の 関数は 台が 知って いるので 押して います★／★中の LAMBDA を 知らないので #NAME? か 1マス扱いに なります★'],
  ['=SUM(BYCOL(F1:G2,LAMBDA(c,SUM(c))))',
    '★ラムダの 一族が まだ★（段取り ⑵〜⑷）＝★外側の 関数は 台が 知って いるので 押して います★／★中の LAMBDA を 知らないので #NAME? か 1マス扱いに なります★'],
  ['=ROWS(MAP(A1:A5,LAMBDA(x,x*2)))',
    '★ラムダの 一族が まだ★（段取り ⑵〜⑷）＝★外側の 関数は 台が 知って いるので 押して います★／★中の LAMBDA を 知らないので #NAME? か 1マス扱いに なります★'],
  ['=SUM(MAP(A1:A5,LAMBDA(x,x*2)))',
    '★ラムダの 一族が まだ★（段取り ⑵〜⑷）＝★外側の 関数は 台が 知って いるので 押して います★／★中の LAMBDA を 知らないので #NAME? か 1マス扱いに なります★'],
  ['=ROWS(SCAN(0,A1:A5,LAMBDA(a,b,a+b)))',
    '★ラムダの 一族が まだ★（段取り ⑵〜⑷）＝★外側の 関数は 台が 知って いるので 押して います★／★中の LAMBDA を 知らないので #NAME? か 1マス扱いに なります★'],
  ['=SUM(SCAN(0,A1:A5,LAMBDA(a,b,a+b)))',
    '★ラムダの 一族が まだ★（段取り ⑵〜⑷）＝★外側の 関数は 台が 知って いるので 押して います★／★中の LAMBDA を 知らないので #NAME? か 1マス扱いに なります★'],
  ['=SUM(MAKEARRAY(2,3,LAMBDA(r,c,r*c)))',
    '★ラムダの 一族が まだ★（段取り ⑵〜⑷）＝★外側の 関数は 台が 知って いるので 押して います★／★中の LAMBDA を 知らないので #NAME? か 1マス扱いに なります★'],
  ['=LET(f,LAMBDA(x,x*2),f(3))',
    '★ラムダの 一族が まだ★（段取り ⑵〜⑷）＝★外側の 関数は 台が 知って いるので 押して います★／★中の LAMBDA を 知らないので #NAME? か 1マス扱いに なります★'],
  ['=LET(a,2,SUM(MAP(A1:A5,LAMBDA(x,x*a))))',
    '★ラムダの 一族が まだ★（段取り ⑵〜⑷）＝★外側の 関数は 台が 知って いるので 押して います★／★中の LAMBDA を 知らないので #NAME? か 1マス扱いに なります★'],
]);
...
function 板を作る() {
  ...
  h.打つ('F1', '1'); h.打つ('G1', '2'); h.打つ('F2', '10'); h.打つ('G2', '20');
  return h;
}
{
  const h = 板を作る();
  h.打つ('J1', '=SUM(A1:A5)');
  T('★材料が 入った★（=SUM(A1:A5) が 15）', String(h.字('J1')) === '15', '出た「' + h.字('J1') + '」');
  const g = 板を作る();
  g.打つ('J1', '=SUM(F1:G2)');
  T('★足した 材料も 入った★（=SUM(F1:G2) が 33）', String(g.字('J1')) === '33', '出た「' + g.字('J1') + '」');
}
...
for (const q of 組) {
  ...
  h.打つ('J1', q.式);
  let 生 = null;
  try { 生 = h.値 ? h.値('J1') : null; } catch (e) { /* 字で 見る */ }
  const 字 = String(h.字('J1'));
