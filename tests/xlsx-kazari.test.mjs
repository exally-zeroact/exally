...
/* ══ ★★濃さ（tint）★★ ══（2026-09-21 経営者1 が 実Excel に 聞きました）
     ...
     ★★ぴったり 合う 式は 見つかって いません★★
       ⇒★差 2までは 出ます★（255段階の うち）＝★門も 差 2で 通します★
       ⇒★★「合って います」と 書かない 代わりに 数で 押さえます★★
       ⇒★差が 3以上に なったら 赤★＝★黙って 悪く ならない★ */
const 濃さの実測 = [
...
T('★★濃さ（tint）が 実Excel と 差 2 以内★★（★ぴったりでは ありません★）', () => {
  const 外 = [];
  for (const [t, 待] of 濃さの実測) {
    const 出 = K.濃さを掛ける('#156082', t);
    const d = 色の差(出, 待);
    if (d > 2) 外.push('t=' + t + ' 出 ' + 出 + ' 実 ' + 待 + ' 差 ' + d);
  }
...
console.log('    ・★★濃さ（tint）は ぴったりでは ありません★★＝★差 2までは 出ます★');
console.log('      ＝実測 9個 中 8個が 差 1以内 ／ t=0.75 だけ 差 2');
