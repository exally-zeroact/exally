...
const EXCLUDED = {
  ...
  /* ★★2026-09-15 に 1本 足した★★＝★★作る前に 書いた 見張り★★
     ★訳★…指示役1 の 注文（「動いた」の 後に 書くと ★通る形に 合わせて しまう★）
     ★今 赤 8本★＝★それが 正しい 姿★。土台が 入った 日に 緑に なります。 */
  'tests/shiki-hyou-itamatagi.test.mjs': {
    reason: '★㋖（板またぎの 土台）を 作る 前に 書いた 見張り★。'
      + '★土台が まだ 無いので 赤 8本★（★出来て いない 事を 隠さない／'
      + 'でも CI を 常態で 赤に しない★）。'
      + '★先に 赤である 事を 見てから 土台を 書く★（「動いた」の 後に 書くと '
      + '★通る形に 合わせて しまいます★）。'
      + '手で 走らせる … node tests/shiki-hyou-itamatagi.test.mjs',
    restoreWhen: '★板またぎの 土台が 入って ★緑に なった 日★'
      + '（その時 この 行を 消して run.js に 載せる）。'
      + '★目安★…実物で 押せる 式が 8,115 → 15,799 に なる 日',
  },
};
