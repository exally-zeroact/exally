 ...
 *    1つも 無ければ ★名前と 中身が 違う★。
 *
 *  ★これは 毎回 走らせる★（tests/run.js に 登録済み）。
 *  ★repo を 1文字も 書き換えない★ので 安全（読むだけ）。
 *
 *  走らせ方: node tests/name-vs-body.test.mjs
 */
import fs from 'node:fs';
import path from 'node:path';

const ROOT = path.join(path.dirname(new URL(import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, '$1')), '..');
const TESTS = ROOT.endsWith('tests') ? ROOT : path.join(ROOT, 'tests');
...
function 壊しているか(中) {
  ...
  /* ★表から 1行 抜く／書き換える 形★（2026-08-30 に 足した）
     ribbon-label / ribbon-scope は ★決まりの 表を その場で 触って 赤を 見ている★のに
     ★この 見張りが その 壊し方を 知らず★「壊していない」と 誤って 言った。
     ⇒ ★見張りの 側が 取りこぼしていた★ので ここを 直す。
     （必ず 元に 戻しているかは 試験の 中の「戻した」の 検査が 見る） */
  if (/delete\s+[^\s;]+\[/.test(中)) return '表から 1行 抜いて 赤を 見ている';
  if (/\[\s*'[^']+'\s*\]\s*=/.test(中)) return '表の 1行を 書き換えて 赤を 見ている';
  return null;
}
